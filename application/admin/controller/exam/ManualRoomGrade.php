...
class ManualRoomGrade extends Backend
{
     ...
     * @var \app\admin\model\exam\ManualRoomGradeModel
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\exam\ManualRoomGradeModel;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $list = $this->model
                    ->with(['admin','user','paper','room','question','grade'])
                    ->where($where)
                    ->order($sort, $order)
                    ->paginate($limit);
            ...
            foreach ($list as $row) {
				...
				$row->getRelation('grade')->visible(['score']);
            }
