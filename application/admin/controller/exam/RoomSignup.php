 ...
 * @icon fa fa-circle-o
 */
class RoomSignup extends Backend
{
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\exam\RoomSignupModel;
    ...
    {
        $room = RoomModel::get($params['room_id']);
        if (!$room) {
            // throw new \Exception('考场信息不存在，请重新选择');
            $this->error('考场信息不存在，请重新选择');
        }

        if ($params['status'] == RoomSignupStatus::ACCEPT) {
            if ($room['people_count'] != 0 && $room['signup_count'] >= $room['people_count']) {
                // throw new \Exception('考场人数已满，无法通过该报名');
                $this->error('考场人数已满，无法通过该报名');
            }
        }
